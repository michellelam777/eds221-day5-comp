...
# The line below creates a sequence of times over which to make the population estimates.
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below creates a placeholder matrix with the columns representing the initial population structure and the rows representing the sequence of times created above. Populations will be added in here.
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below populates the initial population in the first row of the matrix.
proj_year[1, ] <- fish_year0

# The for loop below creates a vector indexing the time_yr vector from the second position to the last. We have already populated time at 0. 
# Take the dot product with the initial years population and add it to row 2 in the proj_year matrix. Loop through taking the dot product of the previous year to fill the matrix rows.
...
# collapse everything except time year column, put names of columns into column called lifestage, put values of the columns into a column called stage_population.
